    ...
    if st.session_state.username == "Federica Giunta":
        with st.sidebar.expander("🛠 PANNELLO DI CONTROLLO", expanded=True):
            ...
            if not asta_bloccata:
                ...
                if st.button("🟢 AVVIA ASTA PER TUTTI", use_container_width=True):
            ...
            if st.button("📊 GENERA REPORT FINALE", use_container_width=True):
                st.session_state.show_report = True

            if st.button("🐷 Premi per elevare la vita di un povero maialino indifeso!", use_container_width=True):
