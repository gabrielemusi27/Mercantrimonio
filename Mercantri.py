...
def get_global_state():
    return {
        "asta_aperta": True,
        "dati_offerte": None,        # Snapshot delle offerte visibile a tutti
        "ultimo_update_time": 0      # Timestamp per la sincronizzazione
    }
...
def forza_scaricamento_offerte():
    """L'admin scarica i dati reali da Google e li mette nello stato globale"""
    ws = sh.worksheet("Offerte")
    df = pd.DataFrame(ws.get_all_records())
    global_state["dati_offerte"] = df
    global_state["ultimo_update_time"] = time.time()
    return df

@st.cache_data(ttl=3600) # Non legge MAI da Google in automatico per risparmiare API
def get_offerte_snapshot():
    if global_state["dati_offerte"] is None:
        return forza_scaricamento_offerte()
    return global_state["dati_offerte"]
...
if 'df_tavoli' not in st.session_state or 'df_carte' not in st.session_state:
    with st.spinner("Sincronizzazione tavoli e carte..."):
        ...
        # Inizializziamo le offerte locali per l'aggiornamento istantaneo dell'utente
...
if not st.session_state.user_logged:
    ...
    # --- PANNELLO ADMIN (Federica Giunta) ---
    if st.session_state.username == "Federica Giunta":
        with st.sidebar.expander("🛠 PANNELLO DI CONTROLLO", expanded=True):
            ...
            # PULSANTE DI REFRESH GLOBALE (La tua idea per salvare le API)
    ...
    # --- ELENCO CARTE CON LOGICA LOCALE ---
    df_db = get_offerte_snapshot()
    
    for i, row in df_carte.iterrows():
        nc = row["Nome Carta"]
        
        # Logica Prezzi (Snapshot Admin vs Offerta Locale)
        prezzo_mostrato = 0
        ...
        # Sovrascrivi se l'utente ha fatto una puntata locale più alta (istantanea)
        ...
        with st.container(border=True):
            ...
            with col_img:
                if row['Immagine']: st.image(row['Immagine'], use_container_width=True)
                else: st.write("🖼️")
            with col_txt:
                st.write(f"### {nc}")
            ...
            with col_btn:
                ...
                if asta_bloccata:
                    ...
                    with st.expander("🚀 Punta", expanded=False):
                        ...
                        nuova = st.number_input("Importo (€)", min_value=int(prezzo_mostrato) + 1, step=1, key=f"in_{chiave}")
                        if st.button("Conferma", key=f"go_{chiave}", use_container_width=True):
                            append_row("Offerte", {
                                "Tavolo": st.session_state.tavolo, "Carta": nc,
                                "Offerta": nuova, "Nome Utente": st.session_state.username
                            })
